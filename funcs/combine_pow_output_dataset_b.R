...
power  <- read.delim(".../dataset_b_pow_full_channels_datanum_1.csv", sep=",",dec= ".") 
power_colnames <- colnames(power)

no_subjects <- 2:118

for(subnr in no_subjects){
  power_subj <- read.delim(paste(".../dataset_b_run_1_pow_full_channels_datanum_",subnr,".csv",sep=""), sep=",", dec= ".")
